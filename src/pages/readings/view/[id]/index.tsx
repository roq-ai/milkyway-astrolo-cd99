...
import { getReadingById } from 'apiSdk/readings';
import { ReadingInterface } from 'interfaces/reading';

function ReadingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ReadingInterface>(
    () => (id ? `/readings/${id}` : null),
    () =>
      getReadingById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('reading', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/readings/edit/${id}`} passHref legacyBehavior>
                    <Button
