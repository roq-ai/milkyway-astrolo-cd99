...
import { getReadings, deleteReadingById } from 'apiSdk/readings';
import { ReadingInterface } from 'interfaces/reading';

type ColumnType = ColumnDef<ReadingInterface, unknown>;
...
export function ReadingListPage(props: ReadingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ReadingInterface>>(
    () => `/readings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ReadingInterface) => {
    if (hasAccess('reading', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/readings/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/readings/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('reading', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/readings/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('reading', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="reading" />
        </Flex>

        {hasAccess('reading', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/readings/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
