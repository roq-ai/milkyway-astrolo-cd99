...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const allowed = await prisma.birthchart
    .withAuthorization({
      roqUserId,
  ...
  async function getBirthchartById() {
    const data = await prisma.birthchart.findFirst(convertQueryToPrismaUtil(req.query, 'birthchart'));
    return res.status(200).json(data);
  }

  async function updateBirthchartById() {
    await birthchartValidationSchema.validate(req.body);
    const data = await prisma.birthchart.update({
  ...
  async function deleteBirthchartById() {
    ...
    const data = await prisma.birthchart.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
