...
import { getBirthchartAnalyses } from 'apiSdk/birthchart-analyses';
...
import { getBirthchartInterpretations } from 'apiSdk/birthchart-interpretations';
import { BirthchartInterpretationInterface } from 'interfaces/birthchart-interpretation';

import { AstrologyBookListPage as ShowCaseList } from 'pages/astrology-books';

function DashboardPage() {
  const { session } = useSession();

  const birthchartAnalysisStat = useSWR<PaginatedInterface<BirthchartAnalysisInterface>>(
    () => `/birthchart-analyses`,
    () => getBirthchartAnalyses({ limit: 1, offset: 0 }),
  );
  ...
  const birthchartInterpretationStat = useSWR<PaginatedInterface<BirthchartInterpretationInterface>>(
    () => `/birthchart-interpretations`,
    () => getBirthchartInterpretations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Birthchart Analyses',
      result: birthchartAnalysisStat?.data?.totalCount,
      isLoading: birthchartAnalysisStat.isLoading,
    ...
    {
      title: 'Birthchart Interpretations',
      result: birthchartInterpretationStat?.data?.totalCount,
      isLoading: birthchartInterpretationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Astrology Books
                </Text>
                <NextLink href={`astrology-books`} passHref legacyBehavior>
                  <Button
